...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const usMentionsKeywords = await db.usMentionsKeyword.findMany({
    where: { countryId: params.id },
    orderBy: { createdAt: "desc" }
  });

  return json<LoaderData>({ 
    usMentionsKeywords: usMentionsKeywords.map(keyword => ({
      ...keyword,
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === "create") {
    ...
    await db.usMentionsKeyword.delete({
      where: { id: keywordId }
    });
  }
...
export default function KeywordsTab() {
  ...
  const { usMentionsKeywords } = useLoaderData<typeof loader>();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    // If there's current input, add it to preview usMentionsKeywords
    if (currentInput.trim()) {
      setPreviewKeywords(prev => [...prev, currentInput.trim()]);
      setCurrentInput("");
    }
    
    // Add all preview usMentionsKeywords to the form data
  ...
  const handleGenerate = async () => {
    ...
    generateFetcher.submit(
      ...
      {
        method: "post",
        action: `/api/countries/${country.id}/generate-us-mentions-keywords`,
      }
    );
  };
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-medium">US Mentions Keywords</h2>
